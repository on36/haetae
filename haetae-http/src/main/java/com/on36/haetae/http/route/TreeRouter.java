...
import static com.on36.haetae.http.route.RouteHelper.getPathElements;
import static com.on36.haetae.http.route.RouteHelper.urlDecodeForRouting;
...
public class TreeRouter implements Router {
	...
	public synchronized void add(Route route) {
		...
		if (!pathElements.isEmpty() && route.endsWithPathSeparator()) {
			pathElements.add(new StaticPathElement(PATH_ELEMENT_SEPARATOR,
					pathElements.size() - 1));
		}

		if (root == null) {
			root = new TreeNode(
					new StaticPathElement(PATH_ELEMENT_SEPARATOR, 0));
		}
	...
	/**
	 * Returns a Route that matches the given URL path. Note that the path is
	 * expected to be an undecoded URL path. The router will handle any decoding
	 * that might be required.
	 * 
	 * @param path
	 *            an undecoded URL path
	...
	public Route route(String path) {
		...
		if (searchTokens.isEmpty() && root.containsSplatChild()
				&& !root.hasRoute()) {
	...
	private List<String> getPathAsSearchTokens(String path) {
		...
		if (!tokens.isEmpty() && path.trim().endsWith(PATH_ELEMENT_SEPARATOR)) {
	...
	public void remove(String resource) {
		...
		if (route != null) {
			...
			if (root == null) {
				root = new TreeNode(new StaticPathElement(
						PATH_ELEMENT_SEPARATOR, 0));
			}

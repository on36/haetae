...
public class PidFile extends Thread {
	...
	static {
		File haetaeHome = new File(System.getProperty("java.io.tmpdir"), "haetae");
		try {
	...
	public void init() throws IOException {
		...
		if (pidDir == null) {
			pidDir = haeataePath + File.separator + "var" + File.separator + "run";
		}
	...
	public void clean() {
		...
		if (!pidFile.exists()) {
			System.out.println("Delete pid file, No such file or directory: " + pidFileName);
		} else {
			try {
				lock.release();
				pidFileOutput.close();
			} catch (IOException e) {
				System.out.println("Unable to release file lock: " + pidFileName);
			}
		}
